...
static int vbus;
static struct regulator *switch_regulator;
...
static void check_vbus_in(struct work_struct *w)
{
	...
	if(pInfo->audio_dock_lock == 0)
		if(pInfo->accessory_type == DOCK_STATE_AUDIO_DOCK && vbus_in == 1) {
#ifdef CONFIG_HTC_HEADSET_MGR
			CABLE_INFO("notify auido driver in check_vbus_in\n");
...
static void cable_detect_handler(struct work_struct *w)
{
	...
	/* accessory = audio dock & vbus = high : lock irq and notify audio driver.
		...
		if (vbus == 1) {
			CABLE_INFO("notify auido driver in cable_detect_handler\n");
			pInfo->audio_dock_lock = 1;
			headset_ext_detect(USB_AUDIO_OUT);
...
static int second_detect(struct cable_detect_info *pInfo)
{
	...
	if ((pInfo->mhl_version_ctrl_flag) || (adc_value >= 776 && adc_value <= 1020))
		...
		type = DOCK_STATE_UNDEFINED;
#endif
/*	else if(adc_value >= 1070 && adc_value <= 1153)*/
	else if(adc_value >= 1021 && adc_value <= 1093)
...
static int cable_detect_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		pInfo->fireball_disable_ldo3 = pdata->fireball_disable_ldo3;
	...
	if (pInfo->fireball_disable_ldo3 == 1) {
		switch_regulator = regulator_get(&pdev->dev, "switch_ldo3");
		if (IS_ERR(switch_regulator)) {
			CABLE_ERR("fail to get switch regulator\n");
			return 0 ;
		}
		if(regulator_set_voltage(switch_regulator, 3075000, 3075000))
			CABLE_ERR("cable probe fail to setup voltage\n");
		if(regulator_enable(switch_regulator))
			CABLE_ERR("cable probe fail to enable switch regulator\n");
		CABLE_INFO("enable switch_ldo3\n");
	}
	return 0;
}
...
static int cable_detect_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct cable_detect_info*pInfo = &the_cable_info;
	if(pInfo->fireball_disable_ldo3 == 0)
		return 0;
	/* if current accessory is usb headset. keep ldo3, don't close */
	if(pInfo->accessory_type == DOCK_STATE_USB_HEADSET)
		return 0;
	if(switch_regulator)
		if(regulator_disable(switch_regulator))
			CABLE_ERR("cable suspend fail to disable switch regulator\n");
	pInfo->usb_dpdn_switch(PATH_POWER_SAVE);
	return 0;
}

static int cable_detect_resume(struct platform_device *pdev)
{
	struct cable_detect_info*pInfo = &the_cable_info;
	...
	if(switch_regulator)
		if(regulator_enable(switch_regulator))
			CABLE_ERR("cable resume fail to enable switch regulator\n");
	if(pInfo->accessory_type == DOCK_STATE_USB_HEADSET)
		pInfo->usb_dpdn_switch(PATH_USB_AUD);
	else
		pInfo->usb_dpdn_switch(PATH_USB);
...
struct platform_driver cable_detect_driver = {
	...
	.suspend = cable_detect_suspend,
	.resume = cable_detect_resume,
