...
static int X_fake_E = 480;
static int Y_fake_E = 600;
static int dx_fake = 1;
static int dy_fake;
static unsigned long report_time = 10000000;

static enum hrtimer_restart atmel_ts_timer_fake_event_func(struct hrtimer *timer)
{
...
static ssize_t atmel_fake_event_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	if (!i) {
		...
		ts->timer.function = atmel_ts_timer_fake_event_func;
...
static ssize_t atmel_fake_event_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	char buf_tmp[5];
	int i = 0, j = 0, k = 0, ret;
...
static DEVICE_ATTR(fake_event, (S_IWUSR|S_IRUGO),
	atmel_fake_event_show, atmel_fake_event_store);
			 ...
			 const struct i2c_device_id *id)
{
	...
	ts->input_dev->mtsize = pdata->config_T9[T9_CFG_NUMTOUCH];
