					 ...
					 const char *buf, size_t count)
{
	...
	wakeup = simple_strtoull(buf, NULL, 10);
	if (wakeup) {
		if (wakeup_bitmask) {
			printk(KERN_INFO "[KEY] %s:already set wake up function\n", __func__);
			return count;
		}
		else
			wakeup_bitmask |= BIT(0);
	} else {
		if (!wakeup_bitmask) {
			printk(KERN_INFO "[KEY] %s:already set non-wake up function\n", __func__);
			return count;
		}
		else
			wakeup_bitmask &= ~BIT(0);
...
{
	return sprintf(buf, "wakeup_music_show:%d\n", wakeup_bitmask);
}
