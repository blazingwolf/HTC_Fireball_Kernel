...
static void android_work(struct work_struct *data)
{
	...
	if (cdev->config) {
		...
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->performance_lock && dev->bEnablePerfLock) {
...
/* For QCT modem transport interface design
 */
					 ...
					 struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata->usb_rmnet_interface)
		strlcpy(rmnet_transports, dev->pdata->usb_rmnet_interface,
			sizeof(rmnet_transports) - 1);
	return 0;
}
...
static int __init init(void)
{
	...
	/* hold a performance lock if gadget driver need */
	dev->bEnablePerfLock = 1;
	_android_dev = dev;
