		...
		uint32_t token, uint32_t *payload, void *priv)
{
	...
	case ASM_DATA_EVENT_WRITE_DONE: {
		...
		pr_debug("[AUD]ASM_DATA_EVENT_WRITE_DONE\n");
		pr_debug("[AUD]Buffer Consumed = 0x%08x\n", *ptrmem);
	...
	case ASM_DATA_CMDRSP_EOS:
		pr_debug("[AUD]ASM_DATA_CMDRSP_EOS\n");
		prtd->cmd_ack = 1;
	...
	case APR_BASIC_RSP_RESULT: {
		...
		case ASM_STREAM_CMD_FLUSH:
			pr_debug("[AUD]ASM_STREAM_CMD_FLUSH\n");
			prtd->cmd_ack = 1;
			wake_up(&the_locks.eos_wait);
			break;
		default:
	...
	default:
		pr_debug("[AUD]Not Supported Event opcode[0x%x]\n", opcode);
		break;
	}
}
...
static int msm_compr_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		pr_debug("[AUD]%s: Trigger start\n", __func__);
		q6asm_run_nowait(prtd->audio_client, 0, 0, 0);
		atomic_set(&prtd->start, 1);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
		pr_debug("[AUD]SNDRV_PCM_TRIGGER_STOP\n");
		atomic_set(&prtd->start, 0);
		break;
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		pr_debug("[AUD]SNDRV_PCM_TRIGGER_PAUSE\n");
